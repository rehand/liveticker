sortFormation = function (formation, onlyPosition) {
    ...
    var getValue = function (value) {
        ...
        if (onlyPosition) {
            ...
            if (pos < 0) {
                if (value.exchanged) {
                    pos = offset + value.exchanged[0].getTime();
                } else {
                    pos = offset + nowOffset + POSITIONS.indexOf(value.position);
                }
            }
        }
...
mapVotableKickers = function (formation, entries) {
    ...
    return sortFormation(ensureArray(formation).filter(function (entry) {
