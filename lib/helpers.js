...
populateFormation = function (formation, tickerEntries) {
    ...
    var kickersScored = eventEntries.filter(function (entry) {
        return (entry.eventType === EVENT_TYPE_GOAL && !entry.varEventId) || entry.eventType === EVENT_TYPE_PENALTY_GOAL;
...
arrayDistinct = function (array) {
    return ensureArray(array).filter((value, index, self) => self.indexOf(value) === index);
};